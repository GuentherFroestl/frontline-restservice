...
public class TaopixKontaktImporterImpl extends KontaktImporterImpl implements TaopixKontaktImporter {
  ...
  private static final Logger LOG = Logger.getLogger(TaopixKontaktImporterImpl.class.getSimpleName());
  ...
  public KontaktSyncDO syncTaopixUser(String userId, HashMap<String, String> propertyListe) {
    ...
    if (kso == null) {
      ...
      if (kso.getKontakt() != null) {
        LOG.info("Synceintrag neu erstellt userID: " + userId + ", KontaktID: " + kso.getKontakt().getId() + " Kontakt: " + kso.getKontakt().getNamenString());
      } else {
        LOG.info("Synceintrag neu erstellt userID: " + userId + ", SyncID: " + kso.getId() + ", Fehler Kontakt == null !!!!!! ");
      }
    } else {
      kontakt = kso.getKontakt();
      if (kso.getKontakt() != null) {
        LOG.info("Synceintrag gefunden userID: " + userId + ", SyncID: " + kso.getId() + ", KontaktID: " + kso.getKontakt().getId() + ", Kontakt: " + kso.getKontakt().getNamenString());
      } else {
        LOG.info("Synceintrag gefunden userID: " + userId + ", SyncID: " + kso.getId() + ", Fehler Kontakt == null !!!!!! ");
      }
  ...
  KontaktDO createKontaktDO(String userId, HashMap<String, String> propertyListe) {
    ...
    if (iso2Code != null
      ...
      if (landNeu != null) {
        ...
        LOG.info("Iso2-Code ist nicht in Land-Tabelle : " + iso2Code);
      }
    }
    adr.setLand(land);
    ...
    LOG.info("createKontaktDO, Kontakt neu erstellt: " + kontakt.getNamenString());
    return kontakt;
  }
